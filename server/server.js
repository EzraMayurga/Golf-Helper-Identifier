...
async function triggerAIAnalysis(videoId, playerId) {
  ...
  try {
    ...
    console.error('[AI Engine] Failed to connect to Python Backend:', error.message);
    const dbErr = readDB();
    const vidErr = dbErr.videos.find(v => v.id === videoId);
    if(vidErr) {
       vidErr.status = 'error';
       writeDB(dbErr);
       broadcast('VIDEO_UPDATE', { videoId, status: 'error' });
    }
  }
}
...
app.post('/api/videos', upload.single('video'), authenticateToken, (req, res) => {
  ...
  const newVideo = {
    ...
    thumbnail: '',
    title: videoTitle,
    videoUrl: file ? `http://localhost:5000/uploads/${file.filename}` : ''
  };
